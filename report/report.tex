...
The feature names are designated using the format "\texttt{category}\_\texttt{featureType}\_\texttt{calculationMethod}", where category is the abbreviated \texttt{category}, \texttt{featureType} is the feature type within the category, (due to page limitation, a complete table would not be provided here, but they are available in the docstring description for each category method here: \cite{ducknewsreporters-nonlatent})\gtododuke{Remove the section in brackets. Also never cite the code, it doesn't make sense. Choose to either include a table with the description or leave this out.}, and \texttt{calculationMethod} is \texttt{sum} when it is the count (i.e. number of) the respective feature, or the default method; it is \texttt{percent} when it is a percentage of all words or unique word in diversity, quantity, or pronoun.\gtododuke{I think this whole paragraph is extremely confusing. Make a list saying "we have these types of features": sum -- this, percent -- that, etc.} Figure \ref{box-plot} is a box plot of all the non-latent features by their scale of power of 10. The majority of features are between the range of $[10^{-2}, 10^{1}]$. Several quantity and some diversity features are in the higher range of $[10^2, 10^3]$.
...
\begin{quote}
  ...
  \[\frac{sumvec(input_{unique} \cap context_{unique})}{sumvec(context_{unique})}\]\gtododuke{what is a sumvec? is it a $\sum$??}
...
\tododuke{MAYBE I COULD PUT THE EXCEL TABLE IN THE APPENDIX OF ALL NON LATENT FEATURES}
