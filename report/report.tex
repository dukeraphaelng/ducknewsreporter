...
To get the context articles, we need to summarize the main topic of our input article down to at most 10 keywords. We use the Python \verb|gensim|~\cite{py-gensim} library which provides various topic modelling interfaces for text inputs. We use the \verb|ldamodel| which implements Latent Dirichlet Allocation (LDA) to extract a single topic. LDA is a probabilistic model where the idea is you have a number of documents representing some latent topics characterized by a distribution over words. \gtodojim{Maybe you could dwell a bit more on how LDA works, and why it is suitable for our usecase, I think LDA is not the only model which can extract summary right, and if possible, you can mention in the limitation and future work some other models that we can potentially use instead of LDA} By feeding in the preprocessed sentences of our input article, we are able to get the main themes. We sort the output keywords by the probability they represent the topic then cap the amount of words to 10 at most.
...
\begin{itemize}
  ...
  \item \emph{Junk} such as connectives would be left in the summary. \gtodojim{Can't we just use spaCy or NLTK to look for all connectives (with the specific POS tags) and remove them? we can also use stop words to remove them, in that case we can add these things as future work, ie to refine the algorithm.}
\end{itemize}
